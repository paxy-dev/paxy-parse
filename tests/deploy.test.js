...
describe("Test deploy", () => {
    ...
    it("shall create config routes page", async () => {
        const generator = new MenuCodeGenerator(
            ["Subject", "Samples", "Box"],
            dir
        );
        generator.create();
        expect(fs.existsSync(path.join(dir, "config", "appRoutes.ts"))).to.be
            .true;
    });
    it("sahll create app code", async () => {
        ...
        expect(fs.existsSync(path.join(config.app.app_page_path, "Subject"))).to
            .be.true;
    });
    it("shall deploy app code from clould run", async () => {
        const table = new Table();
        table.set("name", "Subject");
        await table.save(null, { sessionToken: token });
        ...
        const res = await Parse.Cloud.run("deployAppCode", undefined, {
            sessionToken: token
        });
